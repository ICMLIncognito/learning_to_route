...
class GCNWalkerAgent(SimpleWalkerAgent):
    ...
    def __init__(self, vertex_size, conv_hid_size, hidden_size, output_size, activation=nn.ELU(),
                 convolution_blocks=3, normalize_hid=True, residual_conv=True, **kwargs):
        ...
        self.blocks = [
            GraphConvolutionBlock(vertex_size, hid_size=conv_hid_size, out_size=vertex_size,
                                  normalize_hid=normalize_hid, residual=residual_conv, activation=activation)
    ...
    def prepare_state(self, graph, device='cuda', **kwargs):
        ...
        adj_edges = torch.tensor([(from_i, to_i) for from_i in graph.edges for to_i in graph.edges[from_i]],
                                 device=device)
